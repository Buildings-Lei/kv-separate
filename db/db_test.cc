...
Status GetAllValueLog(std::string dir,std::vector<uint64_t>& logs){
    ...
    std::vector<std::string> filenames;
    // 获取文件列表
    Status s = opts.env->GetChildren(dir, &filenames);
    if (!s.ok()) {
        return s;
    }
    uint64_t number;
...
Status GetTask(DB* db, int start, int end){
    cout<<"---------------begin Get------------------ "<<endl;
    std::string val;
    Status s;
...
Status TestGet(DB* db , int start, int end  ){
    cout<<"---------------begin Get------------------ "<<endl;
    std::string val;
    Status s;
    int error_numbers = 0;
    int delete_numbers = 0;
    for( int i = start; i < end; i++ ){
        if(i == 1923000){
            cout << endl;
        }
        ...
        size_t value_length = read.size();
        s = db->Get(ReadOptions(),key , &val);

        if( !s.ok()  ){
            if (s.IsNotFound()  ) {
                assert( i % 1000 != 0 && i < 500000 );
                continue;
            }
            cout<<"index : "<<i<<" not found "<<endl;
        }
...
Status TestPut( DB* db , int start, int end ){
    cout<<"---------------begin put------------------ "<<endl;
    Status s;
    for( int i = start; i < end; i++ ){
        if( i % 100 == 0){
            s = db->Put(WriteOptions(), to_string(i), to_string(i));
        }else{
            string str = to_string(i);
            str.append(string(512,1));
            s = db->Put(WriteOptions(), to_string(i), str);
        }
        if( !s.ok() ){
            cout<<"index : "<<i<<endl;
            break;
...
Status TestDelet(DB* db , int start, int end ){
    cout<<"---------------begin delete------------------ "<<endl;
    Status s;
    for( int i = start; i < end; i++ ){
        if( i % 1000 != 0){
            s = db->Delete(WriteOptions(), to_string(i));
            assert( s.ok() );
        }
...
void TestPutAndGet(string name ,bool useSnapshot,bool isOpen){
    std::string dbname = testing::TempDir() + name;
    ...
    if( isOpen){
        if( !useSnapshot ){
            if( s.ok()){
                for( int i = 0; i < 5; i++ ){
                    TestPut(db,0,500000);
                    TestDelet(db,0,500000);
                }
                TestGet(db,0,500000);

            }
        }
        TestPut(db,500000,2000000);
    }
    TestPut(db,2000000,2500000);
#endif

    TestGet(db,0 ,2500000);
    uint64_t size = 0;
    GetAllValueLog(dbname,logs);
    for( int i = 0; i < logs.size(); i++ ){
        std::string dir = LogFileName(dbname,logs[i]);
        uint64_t fileSize = 0;
        bool s = GetFileSize(dir,&fileSize);
        if( s ){
            size += fileSize;
        }
    }
    cout<<"befor gc Total log file size:  "<<size<<endl;
    ...
    cout<<"after gc Total log file size:  "<<size<<endl;

    TestGet(db,0,500000);
    cout<<"-----------------begin read log---------------"<<endl;
    logs.clear();
#ifdef ReadLog
    if(s.ok() && GetAllValueLog(dbname,logs).ok()){
        cout<<"get file successfull "<<endl;
        cout<<"Total log is : "<<logs.size()<<endl;
        std::string record_string;
        Slice record;
        cout<<" befor the logs include : ";
        for( int i = 0; i < logs.size(); i++ ){
            cout<<logs[i]<<", ";
        }
        cout<<endl;
        for( int i = 0; i < logs.size(); i++ ){
            cout<<" begin read "<<logs[i]<<endl;
            std::string dir = LogFileName(dbname,logs[i]);
            SequentialFile* file ;

            s = opts.env->NewSequentialFile(dir,&file);
            if( s.ok() ){
                log::VlogReader reader(file,&reporter,true,0);
                uint64_t seq = 0;
                while( reader.ReadRecord(&record,&record_string) ){
                    recordNum++;
                    record.remove_prefix(8);
                    seq = DecodeFixed64(record.data());
                    ...
                    while( record.size() > 0 ){
                        ...
                        numbers++;
                        if( key.ToString() =="1923000" ){
                            cout<<"has  1923000 : "<<endl;
                        }
                        int index = stoi(key.ToString());
                        s = db->Get(ReadOptions(),key,&get_value);
                        // 小于 500000 的值都被删除了
                        if( index < 500000 && index % 1000 != 0 ){
                            assert(s.IsNotFound());
                        }
                        // 大于 500000 的值都要保存着 值还要相等
                        if( index >= 500000  ){
                            assert( !s.IsNotFound() && value.ToString() == get_value );
                        }
                        if( s.IsNotFound() ){
                        ...
                        if (s.IsNotFound() && type != kTypeDeletion) {
                            oldNum++;
                            continue;
                        }

                        seq++;
                        assert( value.ToString() == get_value );
                    }
                }
            }
...
void ThreadTask(DB* db,int start,int end ){
    Status s;
    for( int i = start; i < end; i++ ){
        if( i % 100 != 0){
            s = db->Put(WriteOptions(), to_string(i), to_string(i));
...
void  TestMutilThread(){
    ...
    cout<<"-----------------mutil task--------------"<<endl;
    thread t1(ThreadTask,db,0,500000);
    thread t2(ThreadTask,db,500000,1000000);
    t1.join();
    t2.join();

    thread t3(GetTask,db,0,500000);
    thread t4(GetTask,db,500000,1000000);
    t3.join();
    t4.join();
    cout<<"-----------------------end---------------------"<<endl;
...
int main(int argc, char** argv) {
    ...
    TestPutAndGet("leveldb_test_benchmark_noSnapshot",false,true);
    return 0;
#endif
}
